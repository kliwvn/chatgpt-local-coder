...
try {
  ...
  assert.match(managerServerSource, /if \(p === "\/api\/autostart"\)[\s\S]{0,500}?const state = await inspectAutostartLink\(\)[\s\S]{0,1800}?reconcileAutostartWithLauncher\(enable\)/, "autostart UI must delegate mutation to the canonical launcher and verify shared shortcut authority");
  ...
  assert.match(managerServerSource, /autostartExpectedArguments\(\)[\s\S]{0,220}?startup/, "Manager autostart verifier must require the reconciled startup command");
  assert.match(managerServerSource, /async function reconcileAutostartWithLauncher\(enable\)[\s\S]{0,900}?LAUNCHER_BAT, "autostart"[\s\S]{0,700}?inspectAutostartLink\(\)/, "Manager autostart mutation must use the launcher as the single write authority and verify the result");
