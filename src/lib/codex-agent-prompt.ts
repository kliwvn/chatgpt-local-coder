...
- start_process is intentionally non-terminal. Its result and process_output/process_status expose continuation_required while background work is still running. When continuation_required=true or process_output.running=true, keep polling with process_output until the process reaches running=false, then inspect its exit/output and continue the task. A background job is not a conversation boundary.
